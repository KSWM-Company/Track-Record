...
class Helper
{
    ...
    public static function convertDollarToRiel($amount)
    {
        $data = Currency::where('name','Riel')->orderBy('id','DESC')->first();
        if ($data) {
            return number_format($amount * $data->amount_usd).' '.$data->name;
        }
    }
    public static function convertRielToDollar($amount)
    {
        $data = Currency::where('name','USD')->orderBy('id','DESC')->first();
        if ($data) {
            return round(($amount / $data->amount_riel),2).' '.$data->name;
        }
    }
}
