    ...
    <script>
        function initMap() {
            ...
            var marker = new google.maps.Marker({
                position: {lat: parseFloat({{ $locations->location_latitude }}), lng: parseFloat({{ $locations->location_longitude }})},
                map: map,
                title: {{$locations->id}},
                icon: {
            ...
            let html_img = '';
            let file_img = @json($locations->PreSurveyFile);

            file_img.forEach(element => {
                html_img += `<img src='${element.full_path}' alt='Image' style='width:100px;height:100px;'>`;
            });
            var infowindow = new google.maps.InfoWindow({
                content: "<b> ID: " + {{$locations->id}} + "</b> </br>" + html_img
            });
